        ...
        function loadRoomMovieFrame() {
          ...
          if (video) {
            ...
            video.onloadedmetadata = function() {
              setRoomMovieStatus((isRoomMovieHlsUrl(roomMovieState.proxyVideo) ? "m3u8/HLS loaded." : "Native video loaded.") + " Smooth sync is active with a " + ROOM_MOVIE_DRIFT_LIMIT + "s tolerance.");
            ...
            if (video.src !== roomMovieState.proxyVideo && video.dataset.hlsSrc !== roomMovieState.proxyVideo) {
              roomMovieInitialNativeSyncDone = false;
              roomMovieLastCorrectionAt = 0;
              roomMovieLastPlayPauseAt = 0;
              attachRoomMovieVideoSource(video, roomMovieState.proxyVideo);
            }

            setRoomMovieStatus(isRoomMovieHlsUrl(roomMovieState.proxyVideo) ? "Loading m3u8/HLS in the custom player..." : "Trying native video sync first...");
            setTimeout(function() {
            ...
            loadTimer = setInterval(function() {
              ...
              if (movie.proxyVideo && video) {
                ...
                if (video.src !== movie.proxyVideo) {
                  attachPollingVideoSource(video, movie.proxyVideo);
                }
                setTimeout(function(){

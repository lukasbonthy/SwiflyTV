    ...
    <script>
      (function stableWatchroom(){
        ...
        function updateCustomPlayerUi() {
          ...
          if (progress && dur > 0 && !progress.matches(":active")) {
            progress.value = String(Math.max(0, Math.min(1000, Math.round((cur / dur) * 1000))));
          }
          if (currentTime) currentTime.textContent = customPlayerFormatTime(cur);
          if (duration) duration.textContent = dur > 0 ? customPlayerFormatTime(dur) : "Live";
          ...
          if (playerType) {
            ...
            playerType.textContent = isHls ? "HLS / M3U8 Player" + quality : "Native Video Player" + quality;
          }
        ...
        function loadRoomMovieFrame() {
          ...
          if (video) {
            ...
            video.onloadedmetadata = function() {
              setRoomMovieStatus((isRoomMovieHlsUrl((roomMovieState.playbackUrl || roomMovieState.m3u8 || roomMovieState.proxyVideo)) ? "m3u8/HLS loaded." : "Native video loaded.") + " Smooth sync is active with a " + ROOM_MOVIE_DRIFT_LIMIT + "s tolerance.");
        ...
        function renderRoomMovie(movie) {
          roomMovieState = Object.assign({ status: "idle", movieId: "", playbackUrl: "", proxyVideo: "", m3u8: "", streamType: "", streamQuality: "", streamName: "", playAt: 0, selectedBy: "", message: "", sync: { playing: false, offset: 0, startedAt: 0, updatedAt: Date.now() } }, movie || {});
    ...
    <script>
      (function swiflyDateRoomRecovery(){
        ...
        setTimeout(function(){
          ...
          var movie = { status: "idle", movieId: "", playbackUrl: "", proxyVideo: "", m3u8: "", streamType: "", streamQuality: "", streamName: "", playAt: 0, sync: { playing: false, offset: 0, startedAt: 0 } };
          var timer = null;
          var driftLimit = 5;
        ...
        function startPollingFallback() {
          ...
          function bindPollingCustomPlayer() {
            ...
            function updateUi() {
              ...
              if (duration) duration.textContent = dur > 0 ? formatTime(dur) : "Live";
              if (muteBtn) muteBtn.textContent = video.muted || Number(video.volume || 0) === 0 ? "🔇" : "🔊";
              if (volume && !volume.matches(":active")) volume.value = String(video.muted ? 0 : Number(video.volume || 1));
            ...
            if (progress) progress.addEventListener("input", function(){ var dur = Number(video.duration || 0); if (dur > 0) { try { video.currentTime = (Number(progress.value || 0) / 1000) * dur; } catch {} } updateUi(); });
          ...
          function renderMovie(next) {
            if (!next) return;
            movie = Object.assign({ status: "idle", movieId: "", playbackUrl: "", proxyVideo: "", m3u8: "", streamType: "", streamQuality: "", streamName: "", playAt: 0, sync: { playing: false, offset: 0, startedAt: 0 } }, next);
  ...
  const body = `<main class="dsWatchPage dsM3u8Standalone">
    <section class="dsWatchHero">
      <div>
        ...
        <p>Paste a licensed m3u8 URL as <code>?url=</code>. The player uses native HLS when supported, otherwise hls.js.</p>
      </div>
    </section>
